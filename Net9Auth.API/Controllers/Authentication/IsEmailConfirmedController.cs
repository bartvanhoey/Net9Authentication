...
public class IsEmailConfirmedController(UserManager<ApplicationUser> userManager,  RoleManager<IdentityRole> roleManager,IHostEnvironment environment,
...
#pragma warning restore CS9107 // Parameter is captured into the state of the enclosing type and its value is also passed to the base constructor. The value might be captured by the base class as well.
{
    ...
    public async Task<IActionResult> IsEmailConfirmed()
    {
        try
        {
            ...
            if (email == null) return Nok400Email<IsEmailConfirmedResponse>(logger);

            var user = await userManager.FindByEmailAsync(email);
            if (user == null) return Nok404CouldNotFindUser<IsEmailConfirmedResponse>(logger);
        ...
        {
            return Nok500Exception<IsEmailConfirmedResponse>(logger, exception);
        }
    }
}